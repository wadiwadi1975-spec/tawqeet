...
const translations = {
  ar: {
    common: { loading: 'جارٍ التحميل...', liveData: 'بيانات حية', live: 'حي', cancel: 'إلغاء' },
    ...
    prices: { title: 'أسعار غرامات الذهب — بيانات حية', ounce: 'أوقية', gram24: 'غ 24ق', exchangeRates: 'أسعار الصرف', all: 'الكل', gulf: 'الخليج', levant: 'الشام', other: 'مصر والعراق', karat: 'العيار', loading: 'جارٍ تحميل سعر الذهب...' },
    analysis: { factors: 'العوامل المؤثرة في الذهب', heatmap: 'Heat Map — خريطة الحرارة', previous: 'السابق' },
    ...
    settings: { subscription: 'الاشتراك', mostPopular: 'الأكثر شيوعاً', kdMonth: 'دينار/شهر', liveFx: 'أسعار عملات المنطقة الحية', alerts5: '5 تنبيهات', dailyAI: 'تحليل AI يومي', proHeatmap: 'Heat Map احترافية', subscribeNow: 'اشترك الآن', settingsTitle: 'الإعدادات', currency: 'العملة', displayCurrency: 'عملة عرض الأسعار', language: 'اللغة', autoUpdate: 'تحديث تلقائي', every60s: 'كل 60 ثانية' },
    countries: { kw: 'الكويت', sa: 'السعودية', ae: 'الإمارات', eg: 'مصر', iq: 'العراق' },
  },
  en: {
    common: { loading: 'Loading...', liveData: 'Live Data', live: 'Live', cancel: 'Cancel' },
    ...
    prices: { title: 'Gold Gram Prices — Live Data', ounce: 'oz', gram24: 'g 24K', exchangeRates: 'Exchange rates', all: 'All', gulf: 'Gulf', levant: 'Levant', other: 'Egypt & Iraq', karat: 'Karat', loading: 'Loading gold price...' },
    analysis: { factors: 'Factors Affecting Gold', heatmap: 'Heat Map', previous: 'Previous' },
    ...
    settings: { subscription: 'Subscription', mostPopular: 'Most Popular', kdMonth: 'KD/mo', liveFx: 'Live regional currency rates', alerts5: '5 alerts', dailyAI: 'Daily AI analysis', proHeatmap: 'Professional Heat Map', subscribeNow: 'Subscribe Now', settingsTitle: 'Settings', currency: 'Currency', displayCurrency: 'Price display currency', language: 'Language', autoUpdate: 'Auto Update', every60s: 'Every 60 seconds' },
...
export function I18nProvider({ children }: { children: React.ReactNode }) {
  const [lang, setLangState] = useState<string>(() => {
    try { return localStorage.getItem('twqeet_lang') || 'ar'; } catch { return 'ar'; }
  });
...
export function useI18n() {
  ...
  if (!ctx) return { t: (k: string) => k, lang: 'ar', setLang: () => {}, dir: 'rtl' };
  return ctx;
}
